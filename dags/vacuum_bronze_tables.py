...
def vacuum_tables():
    ...
    if (len(S3_ACCESS_KEY) > 0) and (len(S3_SECRET_KEY) > 0) and (len(S3_END_POINT) > 0):
        ...
        spark = SparkSession.builder \
            .appName("vacuum") \
            .master("local[*]") \
            .config("spark.jars", "http://localhost:9000/warehouse/libs/antlr4-runtime-4.9.3.jar,http://localhost:9000/warehouse/libs/aws-java-sdk-bundle-1.11.901.jar,http://localhost:9000/warehouse/libs/delta-spark_2.12-3.2.0.jar,http://localhost:9000/warehouse/libs/delta-storage-3.2.0.jar,http://localhost:9000/warehouse/libs/hadoop-aws-3.3.1.jar,http://localhost:9000/warehouse/libs/wildfly-openssl-java-1.0.7.Final.jar") \
